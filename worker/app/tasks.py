...
from app.models import (
    ...
    StageDataManager, ComplexityLevel
)
from app.course_content_agent.models import DocumentTree, GroupedLearningPath, GeneratedCourse
from app.course_content_agent.modules import RepoManager, LearningPathGenerator, CourseGenerator
...
def stage1_clone_repository(self, user_id: str, course_id: str, repo_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        stage1_result = Stage1Result(
            ...
            suggested_overview_docs=overview_candidates[:5]  # Limit to top 5
        )
        
        # Save stage data
        stage_data_path = stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.CLONE_REPO, stage1_result
        )
...
def stage2_document_analysis(self, user_id: str, course_id: str, user_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Load Stage 1 data
        stage1_result = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.CLONE_REPO)
        repo_path = Path(stage1_result.repo_path)
        
        # Import CourseBuilder for document processing
        from app.course_content_agent.main import CourseBuilder
        ...
        # Apply LLM analysis
        error_count = builder._apply_llm_analysis(processed_results, tree, overview_context)
        
        # Create stage 2 result
        stage2_result = Stage2Result(
            document_tree_path="",  # Will be set after saving
            processed_files_count=len([r for r in processed_results if r['success']]),
        ...
        tree_path = stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS, tree
        )
        stage2_result.document_tree_path = tree_path
        
        # Save stage result
        stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS, stage2_result
        )
...
def stage3_pathway_building(self, user_id: str, course_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        stage2_result = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS)
        document_tree: DocumentTree = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS)
        ...
        paths_path = stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.PATHWAY_BUILDING, paths_data
        )
        stage3_result.learning_paths_path = paths_path
        
        # Save stage result
        stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.PATHWAY_BUILDING, stage3_result
        )
        ...
        for i, path in enumerate(learning_paths):
            pathway_summaries.append({
                ...
                'modules': [{'title': m.title, 'theme': m.theme} for m in path.modules]
            })
...
def stage4_course_generation(self, user_id: str, course_id: str, user_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Load previous stage data
        stage3_data = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.PATHWAY_BUILDING)
        document_tree: DocumentTree = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS)
        stage2_result = stage_manager.load_stage_data(user_id, course_id, CourseGenerationStage.DOCUMENT_ANALYSIS)
        ...
        from app.course_content_agent.modules import CourseExporter
        ...
        course_path = stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.COURSE_GENERATION, generated_course
        )
        stage4_result.generated_course_path = course_path
        
        # Save stage result
        stage_manager.save_stage_data(
            user_id, course_id, CourseGenerationStage.COURSE_GENERATION, stage4_result
        )
...
def get_stage_status(user_id: str, course_id: str, stage: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not stage_manager.stage_data_exists(user_id, course_id, stage_enum):
            return {
                'exists': False,
                'stage': stage
            }
        ...
        if stage_enum == CourseGenerationStage.CLONE_REPO:
            data = stage_manager.load_stage_data(user_id, course_id, stage_enum)
            return {
                'exists': True,
                'stage': stage,
